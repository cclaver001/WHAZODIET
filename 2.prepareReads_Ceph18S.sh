...
names=(`cut -f 1 /share/projects/OCEAN_eDNA/AZORES/GIT/Ceph18S_samples_20230113.txt`)
R1=(`cut -f 2 /share/projects/OCEAN_eDNA/AZORES/GIT/Ceph18S_samples_20230113.txt`)
R2=(`cut -f 3 /share/projects/OCEAN_eDNA/AZORES/GIT/Ceph18S_samples_20230113.txt`)
x=(`wc -l /share/projects/OCEAN_eDNA/AZORES/GIT/Ceph18S_samples_20230113.txt`)
y=$((x-1))
z=$((x-2))

echo -n "" > fasta_file.txt
echo -n "" > group.txt
echo -n "" > Ceph18S_samples_20230113.fasta
...
for i in `seq 0 $y`; do
	...
	r="$(grep -c '^@A0' ${names[i]}_R1.fastq)"
	echo -n $r >> reads_stats.txt
	echo "" > ${names[i]}.log

#merge pairs

	pear -f ${names[i]}_R1.fastq -r ${names[i]}_R2.fastq -v 20 -m 250 -n 170 -o ${names[i]} >> ${names[i]}.log
	echo -n "," >> reads_stats.txt
...
#Remove forward and reverse primers from assembled sequences (forward and reverse sequences separately)
	...
	cutadapt -g CGCGGCGCTACATATTAGAC...GTCGACGGTCGGTTAAGTGC --discard-untrimmed --minimum-length 100 -e 0.2 -O 17 -o ${names[i]}_cut_fwd.fastq ${names[i]}.assembled_retained.fastq >> ${names[i]}.log
	cutadapt -g GCACTTAACCGACCGTCGAC...GTCTAATATGTAGCGCCGCG --discard-untrimmed --minimum-length 100 -e 0.2 -O 17 -o ${names[i]}_cut_rev.fastq ${names[i]}.assembled_retained.fastq >> ${names[i]}.log
